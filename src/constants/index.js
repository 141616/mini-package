export const ADD = 'ADD'
export const MINUS = 'MINUS'
export const GET_USERINFO = 'GET_USERINFO'
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const LOGIN_FAILE = 'LOGIN_FAILE'
export const CLEAR_LOGIN_ERROR = 'CLEAR_LOGIN_ERROR'

// 仓库
export const GET_PACKAGES_LIST = 'GET_PACKAGES_LIST'
export const GET_PACKAGE_BY_ID = 'GET_PACKAGE_BY_ID'
export const CREATE_PACKAGE = 'CREATE_PACKAGE'
export const DELETE_PACKAGE = 'DELETE_PACKAGE'
export const EDIT_PACKAGE = 'EDIT_PACKAGE'

// 材料
export const ADD_MATERIALS = 'ADD_MATERIALS'
export const GET_MATERIALS_BY_ID = 'GET_MATERIALS_BY_ID'
export const GET_MATERIALS_BY_TYPE = 'GET_MATERIALS_BY_TYPE'
export const GET_STEEL_OF_TOP5 = 'GET_STEEL_OF_TOP5'
export const GET_TOP_MATERIALS = 'GET_TOP_MATERIALS'
